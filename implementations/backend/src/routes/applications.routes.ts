...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const page = Math.max(1, Number.parseInt(typeof req.query.page === 'string' ? req.query.page : '1', 10));
      const limit = Math.min(50, Math.max(1, Number.parseInt(typeof req.query.limit === 'string' ? req.query.limit : '20', 10)));
